...
### Deploy to a public host (Render, free)

`render.yaml` is a complete Render **Blueprint** — push this repo to GitHub,
connect it in Render (*New → Blueprint*), and it provisions PostgreSQL,
Redis, MongoDB, all 9 backend services and the web app automatically. Then
open `https://nexusone-web.onrender.com`. Full walkthrough (including what to
do if a subdomain is taken): **[docs/deployment.md](docs/deployment.md)**.
The platform also runs on Railway, any Docker host, or Kubernetes.
...
│   └── Dockerfile           # shared multi-stage build (SERVICE env selects app)
...
└── .github/workflows/ci.yml # typecheck · tests · builds · GHCR push · deploy hook
```
